...
class UserModel(models.Model):
    ...
    algorithm_type = models.CharField(max_length=20, choices=ALGORITHM_CHOICES, default='knn', help_text="Seçilen makine öğrenmesi algoritması")
    model_file = models.FileField(upload_to='user_models/', help_text="Model dosyası (.pkl)")
    ...
    confusion_matrix = models.JSONField(null=True, blank=True, help_text="Confusion matrix verisi")
    classification_report = models.JSONField(null=True, blank=True, help_text="Classification report verisi")
    class_labels = models.JSONField(null=True, blank=True, help_text="Sınıf etiketleri")
    uploaded_at = models.DateTimeField(auto_now_add=True)
    evaluated_at = models.DateTimeField(null=True, blank=True)

    def __str__(self):
        return f"{self.first_name} {self.last_name} - {self.get_algorithm_type_display()} - {self.uploaded_at.strftime('%Y-%m-%d')}"
